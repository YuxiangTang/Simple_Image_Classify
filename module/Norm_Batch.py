...
train_dir = os.getcwd() + '/data/InputData'
...
def get_files(file_dir, ratio):
    ...
    for file in os.listdir(file_dir + '/elephant'):
        ...
        label_elephant.append(2)
    for file in os.listdir(file_dir + '/rabbit'):
        rabbit.append(file_dir + '/rabbit' + '/' + file)
        label_rabbit.append(3)

    # step2：对生成的图片路径和标签List做打乱处理把cat和dog合起来组成一个list（img和lab）
    image_list = np.hstack((cat, dog, elephant, rabbit))
    label_list = np.hstack((label_cat, label_dog, label_elephant, label_rabbit))
